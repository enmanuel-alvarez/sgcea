<?php
/**
 * Vista: Admin - Crear Sección (Tailwind CSS v3)
 */
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Nueva Sección</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Creación de aula o sección de grupo estudiantil.</p>
    </div>
    <div>
...
<form method="POST" action="<?= url('/admin/secciones/guardar') ?>" class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div>
                ...
                <input type="text" id="nombre" name="nombre" placeholder="Ej: Sección A" required maxlength="50"
            ...
            <div>
                ...
                <input type="number" id="cupo_maximo" name="cupo_maximo" min="1" max="100" value="35"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            </div>
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion" rows="3" maxlength="200" placeholder="Observaciones opcionales..."
