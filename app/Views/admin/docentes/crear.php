<?php
/**
 * Vista: Admin - Crear Docente (Tailwind CSS v3)
 */
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Nuevo Docente</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Registro de información personal y profesional de profesores.</p>
    </div>
    <div>
...
<form method="POST" action="<?= url('/admin/docentes/guardar') ?>" class="space-y-8">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" id="especialidad" name="especialidad" placeholder="Ej: Matemáticas, Ciencias, Lengua..." required maxlength="100"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            </div>
            ...
            <div>
                ...
                <input type="text" id="titulo" name="titulo" placeholder="Ej: Licenciado en Educación" required maxlength="150"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            </div>
        </div>
    </div>
