                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td class="text-right">
                        <div class="flex items-center justify-end space-x-2">
                            <button type="button" onclick="abrirModalCarnet(<?= $doc['id'] ?>, 'docente')" class="p-2 rounded-lg bg-indigo-50 text-indigo-600 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:text-indigo-400 dark:hover:bg-indigo-900/50 transition-colors" title="Ver / Imprimir Carnet">
...
<div id="modalFormDocente" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-2xl p-6 sm:p-8 space-y-6 max-h-[90vh] flex flex-col">
        ...
        <form id="formDocente" method="POST" action="" enctype="multipart/form-data" onsubmit="validarEnvioDocente(event)" class="flex-1 overflow-y-auto pr-1 space-y-5 custom-scrollbar">
            <input type="hidden" name="csrf_token" value="<?= \Src\Core\Security::generarTokenCSRF() ?>">

            <div>
                <label for="doc_foto" class="block text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1.5">Foto de Perfil / Carnet (Opcional)</label>
                <input type="file" id="doc_foto" name="foto" accept="image/png, image/jpeg, image/webp" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white text-xs file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-purple-600 file:text-white hover:file:bg-purple-500">
            </div>
