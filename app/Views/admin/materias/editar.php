<?php
/**
 * Vista: Admin - Editar Materia (Tailwind CSS v3)
 */
...
<div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
    <div>
        <h1 class="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Editar Materia</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Actualización de parámetros de la asignatura.</p>
    </div>
    <div>
...
<form method="POST" action="<?= url('/admin/materias/actualizar/' . $materia['id']) ?>" class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div>
                ...
                <input type="text" id="codigo" name="codigo" value="<?= htmlspecialchars($materia['codigo'] ?? '') ?>" required maxlength="20"
            ...
            <div class="sm:col-span-2">
                ...
                <input type="text" id="nombre" name="nombre" value="<?= htmlspecialchars($materia['nombre'] ?? '') ?>" required maxlength="100"
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="number" id="creditos" name="creditos" value="<?= (int)($materia['creditos'] ?? 1) ?>" min="1" max="20" required
                       class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
            </div>
            ...
            <div>
                ...
                <select id="estado" name="estado"
                        class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm">
                    <option value="1" <?= ($materia['estado'] ?? $materia['activo'] ?? 1) == 1 ? 'selected' : '' ?>>Activa</option>
                    <option value="0" <?= ($materia['estado'] ?? $materia['activo'] ?? 1) == 0 ? 'selected' : '' ?>>Inactiva</option>
                </select>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea id="descripcion" name="descripcion" rows="3" maxlength="500"
                      class="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"><?= htmlspecialchars($materia['descripcion'] ?? '') ?></textarea>
        </div>
    </div>
