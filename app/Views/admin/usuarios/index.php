...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <?php if (in_array('usuarios.crear', $_SESSION['usuario_permisos'] ?? [])): ?>
        ...
        <tbody>
            <?php foreach ($usuarios as $usuario): ?>
                <tr>
                    ...
                    <td>
                        <span class="badge bg-<?= $usuario['tipo'] === 'admin' ? 'danger' : ($usuario['tipo'] === 'docente' ? 'info' : 'success') ?>">
                            <?= ucfirst($usuario['tipo']) ?>
                        </span>
                    </td>
                    <td>
                        <span class="badge bg-<?= $usuario['estado'] == 1 ? 'success' : 'secondary' ?>">
                            <?= $usuario['estado'] == 1 ? 'Activo' : 'Inactivo' ?>
                        </span>
                    </td>
                    <td>
                        <div class="btn-group btn-group-sm">
                            <?php if (in_array('usuarios.editar', $_SESSION['usuario_permisos'] ?? [])): ?>
                                <a href="<?= url('/admin/usuarios/editar/' . $usuario['id']) ?>" class="btn btn-info">
                                    <i class="bi bi-pencil"></i>
                                </a>
                            <?php endif; ?>
                            <?php if (in_array('permisos.asignar', $_SESSION['usuario_permisos'] ?? [])): ?>
                                <a href="<?= url('/admin/permisos/asignar/' . $usuario['id']) ?>" class="btn btn-warning">
                                    <i class="bi bi-key"></i>
                                </a>
                            <?php endif; ?>
                            <?php if (in_array('usuarios.eliminar', $_SESSION['usuario_permisos'] ?? []) && $usuario['id'] !== 1): ?>
